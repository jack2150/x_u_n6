...
function checkUploadAllow($uploadTypeCheck,$hourCheck,$maxUploadFiles,$maxUploadFileSize) {
	...
	$db->setQuery("select size,ip from files where time>=$timeCheck and upload_type=$uploadTypeCheck");
	$db->query();

	if ($db->getNumRows()>=$maxUploadFiles) {
		return false;
	}
	else {
		$fileRows=$db->loadRowList();
		foreach($fileRows as $fileRow) {
			$tempTotalSize+=$fileRow['size'];
		}
		$tempUploadFileSize=$maxUploadFileSize*1024*1024;
		if($tempTotalSize>=$tempUploadFileSize) {
			return false;
		}
		else {
			return true;
...
function buildLinksCode($images)
{
    ...
    foreach($images as $image)
    {
		...
		if (strlen($image[name]) > 48) {
			$image[name] = substr($image[name], 0, 48)."...";
		}
		
		// file type and file icon
		$image[file_icon] = file_icon(getExt($image[name],'.'));
		$image[file_type] = $LANG[FileType][Video];
	...
	if ($input[s] >= 0 and $input[s] <= $info[total]) {
		...
		if ($total_pages==1||$total_pages==0) {
			...
			if ($first_pages != $current_page) {
				$first_pages_href = '<a href="'.$info[baseUrl].'&s='.$first_pages.'">'.$LANG[FirstPage].'</a>';
			}
			
			if ($last_pages != $current_page*$link_number) {
				$last_pages_href = '<a href="'.$info[baseUrl].'&s='.$last_pages.'">'.$LANG[LastPage].'</a>';
			}
			$previous_pages = ($current_page-1)*$link_number;
			$next_pages = ($current_page+1)*$link_number;
			
			if ($previous_pages >= 0) {
				$previous_pages_href = '<a href="'.$info[baseUrl].'&s='.$previous_pages.'">'.$LANG[PreviousPage].'</a>';
			}
			if ($next_pages < $info[total]) {
				$next_pages_href = '<a href="'.$info[baseUrl].'&s='.$next_pages.'">'.$LANG[NextPage].'</a>';
			}
		}
		...
		if ($current_start >= $link_page) {
			...
			$current_start_href = '<a href="'.$info[baseUrl].'&s='.$current_temp.'">'.$LANG[PreviousNumberPage].'</a>';
		}
		
		if ($current_end < $total_pages) {
			$current_temp = $current_end * $link_number;
			$current_end_href = '<a href="'.$info[baseUrl].'&s='.$current_temp.'">'.$LANG[NextNumberPage].'</a>';
		}
		...
		for($temp_link=$current_start;$temp_link<$current_end;$temp_link++) {
			if ($temp_link <= ($total_pages-1)) {
				...
				if ($temp_link == $current_page) {	
					...
					$page_links .= '&nbsp;<a href="'.$info[baseUrl].'&s='.$temp_s.'">'.$temp_link_href.'</a>&nbsp;';
				}
			}
			else {
				break;
