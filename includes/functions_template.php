...
*
* @package phpBB3
* @version $Id: functions_template.php,v 1.14 2006/04/29 17:19:24 acydburn Exp $
...
class template_compile
{
	...
	function compile($code, $no_echo = false, $echo_var = '')
	{
		...
		for ($curr_tb = 0, $tb_size = sizeof($text_blocks); $curr_tb < $tb_size; $curr_tb++)
		{
			...
			switch ($blocks[1][$curr_tb])
			{
				...
				case 'PHP':
					$compile_blocks[] = $this->template->allow_php ? '<?php ' . array_shift($php_blocks) . ' ?>' : '';
				break;
	...
	*/
	function compile_var_tags(&$text_blocks)
	{
		...
		preg_match_all('#\{((?:[a-zA-Z0-9\-_]+\.)+)(\$)?([a-zA-Z0-9\-_]+)\}#', $text_blocks, $varrefs);
	...
	*/
	function compile_write(&$handle, $data)
	{
		...
		$filename = $this->template->cachepath . $this->template->filename[$handle] . '.php';
